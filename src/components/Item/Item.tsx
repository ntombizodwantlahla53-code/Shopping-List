...
import { fetchItem, setItemCatergory, setItemName, setItemNotes,
toggleDropdown } from "../Features/items";
...
export const Item = () => {
    const { items,inputCatergory,inputNotes,inputName ,openIndex } =
    ...
    return (
    <div>
        <Link to = "/home"><div className={style.bc}> <MdArrowBackIos/>
        </div></Link>
        <h1>iiItems ze.. {catergory}</h1>
        <div className={style.itemsContainer}>
            {items.filter((i) =>i.category === catergory)
            .map((item,i) => (<div key={i} className={style.itemRow}>
...
<div className={style.addForm}>
    ...
    placeholder="cater"/>
    <input type="text" value={inputNotes}
    ...
    placeholder="name"/>
    <div className={style.bbtn} onClick ={() => handleDelete()}>
       <Buttons
        type="button"
        label="Delete"
        icon={<IoIosAddCircle />}
